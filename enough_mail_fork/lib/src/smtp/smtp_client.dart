...
class SmtpServerInfo {
  ...
  /// Checks if the server supports DSN (Delivery Status Notification).
  ///
  /// Compare https://tools.ietf.org/html/rfc3461
  bool get supportsDsn => capabilities.contains('DSN');
...
class SmtpClient extends ClientBase {
  ...
  ///
  /// [securityContext] is an optional security context for mTLS (mutual TLS).
  /// Use this to provide client certificates for server authentication.
  SmtpClient(
    String clientDomain, {
    EventBus? bus,
  ...
  /// Set [requestDsn] to `true` to request Delivery Status Notifications
  /// (RFC 3461) - the server will send email notifications about delivery
